...
document.addEventListener('DOMContentLoaded', () => {
  ...
  leadsForm.addEventListener('submit', (e) => {
    ...
    terminalLogs.style.display = 'none'; // Hide telemetry logs container per user request
    ...
    // Trigger visual grid network shake
    window.canvasGridEngine.triggerStrongShake(600, 8);

    // Trigger actual FormSubmit lead sending in background asynchronously without blocking
    const formData = new FormData(leadsForm);
    formData.append('_subject', `New Lead from automationwinners.com: ${name}`);
    
    fetch(leadsForm.action, {
      method: 'POST',
      body: formData,
      headers: {
    ...
    .then(data => {
      console.log('FormSubmit direct delivery success:', data);
    })
    .catch(err => {
      console.error('FormSubmit delivery quiet fallback:', err);
    });

    // Reveal the prominent success message box immediately
    const successBox = document.getElementById('terminal-success-box');
    if (successBox) {
  ...
  terminalCloseBtn.addEventListener('click', () => {
    ...
    // Hide the success message box on exit
    const successBox = document.getElementById('terminal-success-box');
    if (successBox) {
